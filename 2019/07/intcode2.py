...
def run(inputValue, inputValue2, data, state):
    program = data.copy()
    pos = state # use position from state to continue program
    ...
    while True:
        ...
        if instruction["opCode"] == 1:
            ...
            if not previousInput and inputValue:
                op3(inputValue, program[pos + 1], program)
            else:
                op3(inputValue2, program[pos + 1], program)
            previousInput = True
            posIncrement = 2
        elif instruction["opCode"] == 4:
            return op4(program[pos + 1], program, instruction["m1"]), program, pos + 2
